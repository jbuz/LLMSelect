...
def clear_cache():
    ...
    return jsonify({
        ...
        "cleared_at": current_app.extensions.get("cache").cache._cache.clear()
    }), 200
...
def detailed_health():
    ...
    try:
        ...
        health_info["database"]["pool"] = {
            "size": pool.size(),
            "checked_out": pool.checked_out_connections if hasattr(pool, 'checked_out_connections') else None,
            "overflow": pool.overflow() if hasattr(pool, 'overflow') else None,
            "checked_in": pool.size() - pool.checked_out_connections if hasattr(pool, 'checked_out_connections') else None
        }
